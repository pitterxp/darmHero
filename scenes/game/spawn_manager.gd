...
var enemies_to_spawn: int = 0	# gesamtanzahl der gegner in einer runde
var current_wave: int = 0 
var enemy_scenes = [preload("res://scenes/entities/enemies/giardia/giardia.tscn"), preload("res://scenes/entities/enemies/noro/noro.tscn")]
var active_enemies = []
...
func _ready() -> void:
	# Spiel durch Benutzer pausiert?
	if is_instance_valid(get_node("/root/Game")):
		get_node("/root/Game").connect("gameActive", _game_activity_changed)
...
func spawn_enemy() -> void:
	...
	if found_valid_position:
		...
		if enemies_spawned == enemies_to_spawn:
			print("Gegner wurden gespawnt - go!")
...
func _game_activity_changed(new_value: bool) -> void:
	...
	if game_active:
		start_wave_round() # Starte die erste Runde, wenn das Spiel aktiv wird

func start_wave_round() -> void:
	current_wave = 1
	#print("Starte Welle ", current_wave)
